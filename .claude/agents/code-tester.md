...
2. **Boundary cases** — zeros, empty inputs, min/max, exact thresholds,
   negative values, very large/small magnitudes.
3. **Edge / adversarial cases** — invalid types or values, NaN/inf where relevant,
   sign conventions, accumulation/state over many steps,
   and anything the plan's "edge cases & risks" section flagged.
4. **Behavioural/property checks** where useful (e.g. an output that must respond
   in the correct direction to a change in input).
